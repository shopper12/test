...
export function parseMapUrl(href, day={}){
  if(!href)return null;
  try{
    ...
    if(origin&&destination)return{
      kind:"route",origin:normalizePlace(origin,day),destination:normalizePlace(destination,day),
      waypoints:String(p.get("waypoints")||"").split("|").map(x=>normalizePlace(x,day)).filter(Boolean).join("|"),
      mode:p.get("travelmode")||"",source:"map_url_route"
    };
    const q=p.get("query")||p.get("q");
    if(q)return{kind:"place",query:normalizePlace(q,day),source:"map_url_place"};
  }catch{}
  return null;
}
...
export function routeFromText(event, day={}){
  for(const raw of [event?.location,event?.title]){
    const parts=String(raw||"").split(/\s*(?:→|->|⇒)\s*/).map(x=>normalizePlace(x,day)).filter(Boolean);
...
export function placeForEvent(event, day={}){
  return normalizePlace(event?.location||event?.title||"",day);
}

export function endpointForEvent(event, day={}){
  const explicit=parseMapUrl(event?.map_url,day);
...
export function eventMapView(dayEvents,event,day={}){
  const explicit=parseMapUrl(event?.map_url,day);
  if(explicit){explicit.mode=explicit.mode||modeForEvent(event);return explicit;}
  ...
  if(idx>0&&destination){
    for(let i=idx-1;i>=0;i--){
      const origin=endpointForEvent(events[i],day);
      if(origin&&origin!==destination)return{kind:"route",origin,destination,waypoints:"",mode:modeForEvent(event),source:"derived_route",derived:true};
    }
  }
  return{kind:"place",query:destination,source:"place"};
}
...
export function auditEventMappings(events,days){
  ...
  for(const [dayId,dayEvents] of [...grouped.entries()].sort((a,b)=>a[0]-b[0])){
    ...
    const sorted=dayEvents.slice().sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));
    for(const event of sorted){
      const view=eventMapView(sorted,event,day);
      ...
      rows.push({day_id:dayId,id:event.id,title:event.title,mapped,kind:view?.kind||"none",source:view?.source||"none",target,ambiguous:AMBIGUOUS.test(target)});
    }
  }
  return rows;
}
