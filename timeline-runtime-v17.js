...
function openTransit(e){const m=ensureModal(),rows=providerIds(e).map(provider).filter(Boolean);m.querySelector("#dashboard-transit-body").innerHTML=`<div class="transit-modal-event"><b>${esc(e.title)}</b><span>${esc(e.time_start)}~${esc(e.time_end)} · ${esc(e.transport)}</span></div>${rows.map(p=>`<article class="transit-modal-card ${statusClass(p)}"><div class="transit-modal-card-head"><b>${esc(p.name)}</b><span>${esc(statusLabel(p))}</span></div><p>${esc(p.summary||"공식 운행정보 수집 중")}</p><small>최근 수집 ${esc(stamp(p))}</small></article>`).join("")||'<div class="empty">연결된 운영사가 없습니다.</div>'}`;m.hidden=false;document.body.classList.add("transit-modal-open");s.modalEvent=e.id;}
...
function hotelMatches(e){const seed=plan().officialSeed,d=dayById(e.day_id),t=norm(`${e.title} ${e.location}`),rows=seed.hotels||[];const exact=rows.filter(h=>t.includes(norm(h.name))||norm(h.name).includes(t));if(exact.length)return exact;if(!d||!/숙박|호텔|체크인|체크아웃|조식/i.test(`${e.category} ${e.title}`))return[];return rows.filter(h=>d.date>=h.check_in&&d.date<=h.check_out).slice(0,1);}
function restaurantMatches(e){const rows=(plan().officialSeed.restaurants||[]).filter(r=>Number(r.day_id)===Number(e.day_id));const t=norm(`${e.title} ${e.location}`),exact=rows.filter(r=>t.includes(norm(r.name))||norm(r.name).includes(t));return exact.length?exact:(/식사|점심|저녁|조식|맛집/i.test(`${e.category} ${e.title}`)?rows:[]);}
...
function decorateTimeline(){const ev=plan().officialSeed.events||[];document.querySelectorAll("#main-content .event-card[data-event-id]").forEach(card=>{const e=ev.find(x=>String(x.id)===String(card.dataset.eventId));if(!e)return;const body=card.querySelector(":scope > div:nth-child(2)");if(!body)return;body.querySelector(":scope > .all-place-links")?.remove();body.querySelector(":scope > .transit-live-box")?.remove();body.querySelector(":scope > .event-time-breakdown")?.remove();(body.querySelector(".meta")||body.querySelector(".event-title"))?.insertAdjacentHTML("afterend",breakdown(e));const p=placeLinks(e);if(p)body.insertAdjacentHTML("beforeend",p);const t=transitBox(e);if(t)body.insertAdjacentHTML("beforeend",t);mapButton(body,e);});}
function decorateMap(){document.querySelectorAll("#route-list [data-stable-map-event]").forEach(b=>{const e=eventById(b.dataset.stableMapEvent);if(!e)return;let x=b.nextElementSibling;if(!x||!x.classList.contains("map-sidebar-live-extras")){x=document.createElement("div");x.className="map-sidebar-live-extras";b.after(x);}x.innerHTML=`${placeLinks(e,true)}${transitBox(e,true)}`;x.hidden=!x.innerHTML.trim();});}
